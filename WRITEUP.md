...
The Kaggle [Student Performance & Behavior Dataset](https://www.kaggle.com/datasets/mahmoudelhemaly/students-grading-dataset) ships as two files — a *primary* variant and a *biased* variant — of the *same* 5,000-student roster, with contradictory values (departments differ for 74% of students, grades for 82%), so they cannot both be ground truth. Validating each against the data card ([`outputs/validation_report.md`](outputs/validation_report.md)) surfaced the following:
